...
class MVT(nn.Module):
    def __init__(
        ...
        output_stop_token=False,  # 新增：是否添加截止符号
    ):
        super().__init__()
        self.depth = depth
        ...
        self.output_stop_token = output_stop_token  # 新增：截止符号标志
        ...
        if not self.no_feat:
            ...
            if self.rot_ver == 0:
                out_dim = feat_out_size + (2 if self.output_arm_flag else 0) + (2 if self.output_stop_token else 0)
                ...
                feat_out_size_ex_rot = feat_out_size_ex_rot + (2 if self.output_arm_flag else 0) + (2 if self.output_stop_token else 0)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if img_demonstration.dtype == torch.uint8:
            img_demonstration = img_demonstration.float() / 255.0
        else:
            img_demonstration = img_demonstration.float()
        img_demonstration = torch.nn.functional.interpolate(
            img_demonstration, size=(224, 224), mode='bilinear', align_corners=False
        )
        
        for i,img_buf in enumerate(images):
            img_demonstration_buf = MVT.trans_cuda_tensor_2_PIL(img_demonstration[i])
            images[i].append(img_demonstration_buf)
        ...
        for i in range(bs):
            ...
            assert non_zero_output.shape[0] > 256*(self.num_img+1)
            non_zero_output = non_zero_output[:256*(self.num_img+1)]
        ...
        x = rearrange(image_tokens, 'b (c h1 h2) w -> b w c h1 h2', c=self.num_img + 1, h1=self.num_pat_img, h2=self.num_pat_img) 
