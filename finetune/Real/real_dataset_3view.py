...
b_base_transform_matrix = np.array([[ 1.0000000e+00 ,0.0000000e+00,  0.0000000e+00,-2.0000000e-01],
...
def euler_to_vector(euler_angles, degrees=True):
    """
    将欧拉角转换为方向向量
    
    参数:
    euler_angles: [rx, ry, rz] 欧拉角 (度或弧度)
    degrees: 是否使用度为单位
    
    返回:
    direction_vector: [x, y, z] 单位方向向量
    """
    # 创建Rotation对象
    ...
    # 应用旋转得到新的方向向量
    direction_vector = rotation.apply(initial_direction)
...
class Real_Dataset(torch.utils.data.Dataset):
    ...
    def convert_pcd_to_base(
        ...
        # pcd = (transform @ pcd.T).T[:, :]

        pcd = (b_base_transform_matrix @ pcd.T).T[:, :3]
            ...
            for task in os.listdir(data_path):
                ...
                if os.path.isdir(task_path): 
                    for episode_num in tqdm(os.listdir(task_path)):
                        ...
                        # 去掉zed相关路径，改为net、usb1、usb2
                        rgb_net = os.path.join(episode_path, "net_rgb")
                        pcd_net = os.path.join(episode_path, "net_pcd")
                        rgb_usb1 = os.path.join(episode_path, "usb1_rgb")
                        pcd_usb1 = os.path.join(episode_path, "usb1_pcd")
                        rgb_usb2 = os.path.join(episode_path, "usb2_rgb")
                        pcd_usb2 = os.path.join(episode_path, "usb2_pcd")
                        gripper_pose = read_action_file(action_path)
                        

                        num_steps = sum(1 for file_name in os.listdir(rgb_net) if file_name.endswith('.pkl')) 
                        ...
                        for step in range(num_steps - 1):
                            ...
                            # gripper_pose_vector = euler_to_vector(gripper_pose_euler)*0.24 # 0.028m

                            # gripper_pose_xyz = gripper_pose_xyz + gripper_pose_vector
                            
                            # 处理位置坐标
                            gripper_pose_xyz = np.concatenate((gripper_pose_xyz.reshape((1,-1)), np.ones((1, 1))), axis=1)
                            gripper_pose_xyz = (b_base_transform_matrix @ gripper_pose_xyz.T).T[:, :3]
                            gripper_pose_xyz = gripper_pose_xyz.reshape(-1)
                            # 处理欧拉角 - 方法3：使用scipy的Rotation类
                            from scipy.spatial.transform import Rotation as R

                            # 创建原始欧拉角的Rotation对象
                            original_rotation = R.from_euler('xyz', gripper_pose_euler, degrees=True)

                            # 创建变换矩阵的Rotation对象
                            transform_rotation = R.from_matrix(b_base_transform_matrix[:3, :3])

                            # 组合旋转：新旋转 = 变换旋转 × 原始旋转
                            new_rotation = transform_rotation * original_rotation


                            # 转换为四元数
                            gripper_pose_quat = new_rotation.as_quat()
                            ...
                            # 初始化相机字典
                            sample["net"], sample["usb1"], sample["usb2"] = {}, {}, {}
                            
                            if "net" in self.cameras:
                                with open(os.path.join(rgb_net, f"{step}.pkl"), 'rb') as f:
                                    sample["net"]["rgb"] = pickle.load(f)[:, :, :3]
                                    h,w,c = sample["net"]["rgb"].shape
                                    sample["net"]["rgb"] = np.ascontiguousarray(sample["net"]["rgb"])  #   check it  the final image should be RGB
                                    sample["net"]["rgb"] = np.transpose(sample["net"]["rgb"], [2, 0, 1])  # 转为（C,H,W）
                                    rgb_data = sample["net"]["rgb"]
                                with open(os.path.join(pcd_net, f"{step}.pkl"), 'rb') as f:
                                    sample["net"]["pcd"] = pickle.load(f)[:,:3]
                                    sample["net"]["pcd"] = self.convert_pcd_to_base(pcd=sample["net"]["pcd"], type="net",extrinsic_path=os.path.join(episode_path, "extrinsic_matrix.pkl"),h=h,w=w)
                                    sample["net"]["pcd"] = np.transpose(sample["net"]["pcd"], [2,0,1]).astype(np.float32)
                            
                            if "usb1" in self.cameras:
                                with open(os.path.join(rgb_usb1, f"{step}.pkl"), 'rb') as f:
                                    sample["usb1"]["rgb"] = pickle.load(f)[:,:,:3]
                                    h,w,c = sample["usb1"]["rgb"].shape
                                    sample["usb1"]["rgb"] = np.ascontiguousarray(sample["usb1"]["rgb"])  #   check it  the final image should be RGB
                                    sample["usb1"]["rgb"] = np.transpose(sample["usb1"]["rgb"], [2, 0, 1])  # 转为（C,H,W）
                                    rgb_data = sample["usb1"]["rgb"]
                                with open(os.path.join(pcd_usb1, f"{step}.pkl"), 'rb') as f:
                                    sample["usb1"]["pcd"] = pickle.load(f)[:,:3]
                                    sample["usb1"]["pcd"] = self.convert_pcd_to_base(pcd=sample["usb1"]["pcd"], type="usb1",extrinsic_path=os.path.join(episode_path, "extrinsic_matrix.pkl"),h=h,w=w)
                                    sample["usb1"]["pcd"] = np.transpose(sample["usb1"]["pcd"], [2,0,1]).astype(np.float32)
                            
                            if "usb2" in self.cameras:
                                with open(os.path.join(rgb_usb2, f"{step}.pkl"), 'rb') as f:
                                    sample["usb2"]["rgb"] = pickle.load(f)[:, :, :3]
                                    h,w,c = sample["usb2"]["rgb"].shape
                                    sample["usb2"]["rgb"] = np.ascontiguousarray(sample["usb2"]["rgb"])  #   check it  the final image should be RGB
                                    sample["usb2"]["rgb"] = np.transpose(sample["usb2"]["rgb"], [2, 0, 1])  # 转为（C,H,W）
                                    rgb_data = sample["usb2"]["rgb"]
                                with open(os.path.join(pcd_usb2, f"{step}.pkl"), 'rb') as f:
                                    sample["usb2"]["pcd"] = pickle.load(f)[:,:3]
                                    sample["usb2"]["pcd"] = self.convert_pcd_to_base(pcd=sample["usb2"]["pcd"], type="usb2",extrinsic_path=os.path.join(episode_path, "extrinsic_matrix.pkl"),h=h,w=w)
                                    sample["usb2"]["pcd"] = np.transpose(sample["usb2"]["pcd"], [2,0,1]).astype(np.float32)
